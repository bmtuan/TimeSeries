...
class LSTM_Decoder(nn.Module):
    def __init__(self, input_size, output_size, hidden_size, num_layers, mode,dropout=0):
        ...
        self.fc2_1 = nn.Linear(hidden_size, 512)
        self.fc2_2 = nn.Linear(512, 256)
        self.fc2_3 = nn.Linear(256, 64)
        self.fc2_4 = nn.Linear(64, 10)
        self.fc2_5 = nn.Linear(10, 1)
        self.Relu = nn.ReLU()
    ...
    def forward(self, x, state):
        ...
        output_2 = self.fc2_4(output)
        output_2 = self.Relu(output_2)
        out_prob = torch.sigmoid(self.fc2_5(output_2))

        return out_pm, out_prob, hidden_state


class LSTM(nn.Module):
    def __init__(self,input_seq_len,output_seq_len,confidence,number_layer,input_size,output_size,hidden_size,mode):
        ...
        self.decoder = LSTM_Decoder(self.input_size, self.output_size, self.hidden_size, self.number_layer, self.mode)

    def forward(self, x):
        # print(x.shape)
        if self.mode == 'normal':
            outputs = torch.zeros(x.shape[0], self.output_seq_len, x.shape[2])
        else:
            outputs = torch.zeros(x.shape[0], self.output_seq_len, x.shape[2] - 1)
        ...
        for t in range(self.output_seq_len):
            out_linear, out_prob, decoder_hidden = self.decoder(decoder_input, decoder_hidden)
    ...
    def predict(self, iterator, sc_test, confidence, name):
        y_linear_original = []
        y_linear_predict = []
        ...
        with torch.no_grad():
            for x, y1, y2 in iterator:
                ...
                linear_outputs, prob_output = self.forward(x)  # batch_size, output_seq, num_feature
                linear_outputs, prob_output = linear_outputs.to(device), prob_output.to(device)
                y_linear_predict.append(linear_outputs.detach().cpu().numpy()[0, :, 0].reshape(-1))
                y_linear_original.append(y1.detach().cpu().numpy()[0, :, 0].reshape(-1))

                prob_output = prob_output > (1 - confidence)
                prob_output = prob_output.long()

                y_classcification_predict.append(prob_output.detach().cpu().numpy()[:, 0, :].reshape(-1))
                y_classcification_original.append(y2.detach().cpu().numpy()[:, 0, :].reshape(-1))
        # print(np.array(y_linear_predict).shape)
        ...
        precision = precision_score(y_classcification_predict, y_classcification_original)
        ...
        print(f"R2: {r2}")
        plot_results(y_original,y_predict,'/media/aimenext/disk1/tuanbm/TimeSerires/model/output',f'test_batch_{name}.png')
