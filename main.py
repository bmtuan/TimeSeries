...
def prepare(input_path, synthetic_threshold, synthetic_sequence_length, input_len, output_len):
    ...
    train_dataset = PMDataset(
        train_df, input_len=input_len, output_len=output_len)
    valid_dataset = PMDataset(
        valid_df, input_len=input_len, output_len=output_len)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-in', '--input_path',
                        help='path of file csv', type=str, default='data/final_envitus.csv')
    parser.add_argument('-ep', '--epochs',
                        help='Number of training epochs', type=int, default=30)
    parser.add_argument('-in_seq', '--input_seq_len',
                        help='input sequence length', type=int, default=240)
    parser.add_argument('-out_seq', '--output_seq_len',
                        help='output sequence length', type=int, default=60)
    parser.add_argument('-lr', '--learning_rate',
                        help='learning rate', type=float, default=0.0005)
    parser.add_argument('-coef', '--coefficient_loss',
                        help='coefficent loss', type=float, default=0.01)
    parser.add_argument('-path', '--model_path',
                        help='path of save model', type=str, default='checkpoint/test_1004.pth')
    ...
    model = lstm_seq2seq(input_seq_len=args.input_seq_len,
